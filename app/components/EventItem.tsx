...
export default function EventItem({ item, eventItems, setEventItems, setNeedsUpdate, setMessage }
  ...
  const [hour, setHour] = useState(new Date(item.datetime).getHours().toString());
  const [mins, setMins] = useState(new Date(item.datetime).getMinutes().toString());
  ...
  return (
    <li className="item-wrap" style={editEnabled ? {border: '1px solid red'} : {}}>
      ...
      <div className="edit-datetime">
        ...
        <input className="edit-input hour" type="text" name="hour" 
          defaultValue={hour}
          onChange={(e) => setHour(e.target.value)} /> 
        <input className="edit-input min" type="text" name="mins" 
          defaultValue={mins}
